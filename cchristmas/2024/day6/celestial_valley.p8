...
player={
	...
	update=function(this)
		...
		elseif this.crouch then
			if dash then
				if contains(inventory,45) and bag.seeds>0 then
					-- consume bait
					bag.seeds-=1
...
tiles={
	...
	[36]={
		...
		draw_tooltip=function(x,y,failed)
			print_outline("$1000",x-2,y+sin(frames/30)-17,score>=60 and 11 or 8,failed and 2 or 0)
		end,
	},
	[239]={
		-- shovel
...
function tick_tile(tx,ty)
	...
	-- meteor spawn
	if ty==0 and num==0 and flr(rnd(128))==0 and not in_cutscene() then
